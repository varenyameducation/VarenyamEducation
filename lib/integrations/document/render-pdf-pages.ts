...
// under the hood, no native canvas dependency). Output PNGs are intended as
// input to Gemini Vision — see lib/integrations/document/parse-page-image.ts.
//
// Each page is rendered at `scale` (default 2 ≈ 150 DPI, enough resolution
// for board-paper math). If a rendered page exceeds the Gemini 5 MiB
// inline-image cap we retry at progressively lower scales (1.5, 1.0) until
// we're under cap; if even scale-1.0 exceeds the cap we drop the page and
